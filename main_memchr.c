...
/*   Updated: 2024/01/14 17:07:55 by pbenito-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(void)
{
	...
	size_t	input_n[SIZE] = {0, 1, 1, 2, 2, 2, 3, 3, 3, 2, 4};
	char	*expected[SIZE];
	char	*result;
	int i;
	...
	while (i < SIZE)
	{
		result = ft_memchr((const void *)input_str[i], input_chr[i], input_n[i]);
		if (result == expected[i])
			set_green();
		else
			set_red();
		printf("Input str: \"%s\"; Input char: %c; Input n: %zu; Result: \"%s\"; Expected: \"%s\"\n", input_str[i], input_chr[i], input_n[i], result, expected[i]);
		i++;
	}
	reset_color();
}
